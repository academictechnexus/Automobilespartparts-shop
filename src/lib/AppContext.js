...
export const AppProvider = ({ children }) => {
  ...
  const [shop, setShopState] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`shop_${tenantId}`) || 'null'); } catch {}
    return { name:'My Auto Spares', address:'123 Main Road', phone:'9876543210', email:'', gstin:'', pan:'', bank_name:'', bank_account:'', ifsc_code:'', invoice_prefix:'INV', purchase_prefix:'PUR', job_prefix:'JOB', state:'Tamil Nadu', state_code:'33', invoice_counter:6, purchase_counter:3, job_counter:3 };
  });

  const load = (key, fallback) => { try { const d = localStorage.getItem(`${key}_${tenantId}`); return d ? JSON.parse(d) : fallback; } catch { return fallback; } };
  const save = (key, data) => localStorage.setItem(`${key}_${tenantId}`, JSON.stringify(data));
  ...
  const saveShop = (s) => { setShopState(s); localStorage.setItem(`shop_${tenantId}`, JSON.stringify(s)); };
  ...
  const getHealthScore = () => {
    ...
    return { score, grade, details:[
      ...
      {label:'Outstanding Control',value:outstanding.toFixed(1)+'%',color:outstanding<20?'text-green-400':'text-red-400'},
    ]};
  };
  ...
  const upsertPart = (part) => {
    ...
    setParts(prev => { const ex=prev.find(p=>p.id===part.id); if(ex){auditLog('UPDATE','parts',id,ex,part);return prev.map(p=>p.id===part.id?{...p,...part}:p);} auditLog('CREATE','parts',id,null,part);return[{...part,id},...prev]; });
  };
  const deletePart=(id)=>{ const item=parts.find(p=>p.id===id); if(item)softDelete('parts',item); setParts(prev=>prev.filter(p=>p.id!==id)); };

  const upsertCustomer = (cust) => {
    const id=cust.id||`c_${Date.now()}`;
    setCustomers(prev=>{ const ex=prev.find(c=>c.id===cust.id); if(ex){auditLog('UPDATE','customers',id,ex,cust);return prev.map(c=>c.id===cust.id?{...c,...cust}:c);} auditLog('CREATE','customers',id,null,cust);return[...prev,{...cust,id}]; });
  };
  const deleteCustomer=(id)=>{ const item=customers.find(c=>c.id===id); if(item)softDelete('customers',item); setCustomers(prev=>prev.filter(c=>c.id!==id)); };

  const upsertSupplier = (sup) => {
    const id=sup.id||`s_${Date.now()}`;
    setSuppliers(prev=>{ const ex=prev.find(s=>s.id===sup.id); if(ex){auditLog('UPDATE','suppliers',id,ex,sup);return prev.map(s=>s.id===sup.id?{...s,...sup}:s);} auditLog('CREATE','suppliers',id,null,sup);return[...prev,{...sup,id}]; });
  };
  const deleteSupplier=(id)=>{ const item=suppliers.find(s=>s.id===id); if(item)softDelete('suppliers',item); setSuppliers(prev=>prev.filter(s=>s.id!==id)); };

  const addInvoice = (inv) => {
    auditLog('CREATE','invoices',inv.id,null,{invoice_no:inv.invoice_no,total:inv.grand_total});
    (inv.items||[]).forEach(item=>{ if(item.part_id) setParts(prev=>prev.map(p=>p.id===item.part_id?{...p,stock:Math.max(0,p.stock-item.qty)}:p)); });
    setInvoices(prev=>[{...inv,tenant_id:tenantId},...prev]);
  };
  const updateInvoice=(id,updates)=>{ auditLog('UPDATE','invoices',id,null,updates); setInvoices(prev=>prev.map(i=>i.id===id?{...i,...updates}:i)); };
  const cancelInvoice=(id,reason)=>{
    auditLog('UPDATE','invoices',id,null,{action:'cancel',reason});
    const inv=invoices.find(i=>i.id===id);
    (inv?.items||[]).forEach(item=>{ if(item.part_id) setParts(prev=>prev.map(p=>p.id===item.part_id?{...p,stock:p.stock+item.qty}:p)); });
  ...
  const addPurchase = (pur) => {
    ...
    (pur.items||[]).forEach(item=>{ if(item.part_id) setParts(prev=>prev.map(p=>p.id===item.part_id?{...p,stock:p.stock+item.qty}:p)); });
  ...
  const addReturn = (ret) => {
    ...
    (ret.items||[]).forEach(item=>{ if(item.part_id) setParts(prev=>prev.map(p=>p.id===item.part_id?{...p,stock:ret.return_type==='sales_return'?p.stock+item.qty:Math.max(0,p.stock-item.qty)}:p)); });
    setReturns(prev=>[{...ret,id,tenant_id:tenantId,return_no:returnNo},...prev]);
  };

  const upsertJobCard=(job)=>{
    const id=job.id||`job_${Date.now()}`;
    setJobCards(prev=>{ const ex=prev.find(j=>j.id===job.id); if(ex)return prev.map(j=>j.id===job.id?{...j,...job}:j); return[{...job,id,tenant_id:tenantId},...prev]; });
  };

  const addExpense=(exp)=>{ const id=`exp_${Date.now()}`; auditLog('CREATE','expenses',id,null,exp); setExpenses(prev=>[{...exp,id,tenant_id:tenantId},...prev]); };
  ...
  return (
    <AppContext.Provider value={{
      lang, setLang:saveLang, t, shop, setShop:saveShop, tenantId,
